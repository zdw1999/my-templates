...
#define MY_MATH_H
	...
	namespace zdw{
		...
		//inline ll mul(const ll &x,const ll &y,const ll &z){return x*y-(ll)(x/(LD)z*y+1e-3)*z;}//O(1) mul
		...
		inline ll pow(ull a,ull b,const ull &p){ll ret=0;do{if(b&1)ret=mul(ret,a,p);a=mul(a,a,p);}while(b>>=1);return ret;}
		inline ll exgcd(const ll &a,const ll &b,ll &x,ll &y){return b?exgcd(b,a%b,y,x)+(y-=a/b*x)-y:(x=1,y=0,a);}//ax-by=1
			...
			#if PRIME_LIST_LIMIT>100000000
			# undef PRIME_LIST_LIMIT
			# define PRIME_LIST_LIMIT 100000000
			#endif
		...
		bool isp(const ll &n){
			...
			ll a,x=1,y,u=n-1; int t=0,i=CHECK_TIMES+1,j;
			while(!(u&1))++t,u>>=1;
			while(j=t+1,x==1&&--i){
				a=rand()%(n-1)+1; x=pow(a,u,n);
				while(--j)if(x!=1&&x!=n-1&&(x=mul(x,x,n))==1)return 0;
			} return x==1;
		}
		#undef CHECK_TIMES
